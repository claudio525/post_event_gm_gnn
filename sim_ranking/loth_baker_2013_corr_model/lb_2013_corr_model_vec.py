...
from .lb_2013_corr_model import B1, B2, B3, T_list, B_coeff1_interpolator, B_coeff2_interpolator, B_coeff3_interpolator


def get_correlations(T1: np.ndarray[float], T2: np.ndarray[float], h: np.ndarray[float]) -> np.ndarray[float]:
    """
    ...
    if np.any(T1 < 0.01) or np.any(T1 > 10) or np.any(T2 < 0.01) or np.any(T2 > 10):
        raise ValueError(
            "All periods must be between 0.01 and 10 seconds"
        )
    ...
    if equal_indices_mask.any():
        B_coeff1[equal_indices_mask] = _interpolate_B_coefficients(
            T1[equal_indices_mask], T2[equal_indices_mask], ind1[equal_indices_mask], ind2[equal_indices_mask], B1)
        B_coeff2[equal_indices_mask] = _interpolate_B_coefficients(
            T1[equal_indices_mask], T2[equal_indices_mask], ind1[equal_indices_mask], ind2[equal_indices_mask], B2)
        B_coeff3[equal_indices_mask] = _interpolate_B_coefficients(
            T1[equal_indices_mask], T2[equal_indices_mask], ind1[equal_indices_mask], ind2[equal_indices_mask], B3)
        
    # Spatial cross correlation
    B_coeff1[~equal_indices_mask] = B_coeff1_interpolator(np.stack([T1, T2], axis=1))[~equal_indices_mask]
    B_coeff2[~equal_indices_mask] = B_coeff2_interpolator(np.stack([T1, T2], axis=1))[~equal_indices_mask]
    B_coeff3[~equal_indices_mask] = B_coeff3_interpolator(np.stack([T1, T2], axis=1))[~equal_indices_mask]
    
    # Compute the correlation
    rho = B_coeff1[:, None] * np.exp(-3 * h / 20.0) + B_coeff2[:, None] * np.exp(-3 * h / 70.0) + np.where(np.isclose(h, 0.0), B_coeff3[:, None], 0.0)
    return rho


def _interpolate_B_coefficients(T1: np.ndarray[float], T2: np.ndarray[float], ind1: np.ndarray[int], ind2: np.ndarray[int], B: np.ndarray):
    ...
    B_diag_val = [np.interp(
        T_avg_targ[i], T_avg_vals[i], B_avg_vals[i]
    ) for i in range(T1.size)]

    # Interpolate between the diagonal and the corner value of B
    T_diff_targ = np.abs(T1 - T2)

    T_diff_vals = np.stack([np.zeros(T1.size), np.abs(np.min(T1_vals, axis=1) - np.max(T2_vals, axis=1))], axis=1)


    B_diff_vals = np.stack([B_diag_val, B_vals[:, 0, 1]], axis=1)
    B_coeff = [np.interp(
        T_diff_targ[i], T_diff_vals[i], B_diff_vals[i]
    ) for i in range(T1.size)]
